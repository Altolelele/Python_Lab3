...
res_data = json.load(open("result_31.txt", encoding="windows-1251"))
print(*res_data[:10], sep='\n')
while True:
    print("1. Сортировать по полю 'passport_number'")
    print("2. Сортировать по полю 'age'")
    print("0. Выйти из программы")
    cmd = input("Выберите пункт: ")

    if cmd == "1":
        print("Сортировка по полю 'passport_number'")
        sort_data = shell_sort(res_data, "passport_number")
        ...
        print("Сортировка по полю 'age'")
        sort_data = shell_sort(res_data, "age")
        print(*sort_data[:10], sep='\n')
        serialization_file("sort_data.yaml", sort_data)
        print("Сериализация завершена")
        tmp_data = deserialization_file("sort_data.yaml")
